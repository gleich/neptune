...
build-docker-prod:
	docker build -f docker/Dockerfile -t mattgleich/neptune:latest .
build-docker-dev:
	docker build -f docker/dev.Dockerfile -t mattgleich/neptune:test .
build-docker-dev-lint:
	docker build -f docker/dev.lint.Dockerfile -t mattgleich/neptune:lint .
build-go:
	go get -v -t -d ./...
	go build -v .
	rm neptune
